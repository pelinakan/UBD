...
namespace pol_util
{
  ...
  /**
   * @brief For now, only strings that are equal in length
   */
  int hamm_dist(std::string ref, std::string needle, std::string qual="") {
    int dist = 0;
    if (qual.size() == needle.size()) {//We have a quality bound string. Add up quality diff. Solexa >1.3
